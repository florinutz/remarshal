...
// StringMapper can parse a string for its values
...
func Split(text string, splitter interface{}) (map[string]string, error) {
	...
	default:
		if unknownMapper, ok := splitter.(StringMapper); ok {
			spl = unknownMapper
		} else {
...
// Implementing the StringMapper interface for the func
